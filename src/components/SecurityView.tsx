...
import { translateSecurityText } from '../utils/localeRuntime';
import { generateNextInventoryNumber } from '../utils/equipmentFields';
import { canUseBackup, getLicenseStatus } from '../utils/license';
...
export default function SecurityView({
  ...
  const handleApplyPatch = (vulnId: string, patchId?: string, category?: string) => {
    if (category === 'passwords' && patchId) {
      ...
      if (!canUseBackup(getLicenseStatus())) {
        alert(
          t(
      ...
      const generatedInv = generateNextInventoryNumber('PC-SEC', {
        warehouseItems,
        computers,
        networkDevices,
      });
    ...
    <div id="security-audit-container" className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        ...
        >
          ...
          {isScanning ? t('Идет сканирование...') : t('Запустить Тест Защищенности')}
        </button>
      </div>
          ...
          <div className="bg-[#0f172a]/60 border border-slate-800 p-6 rounded-3xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <span className="text-[10px] text-slate-400 font-mono">
                {lastScanTime ? translateSecurityText(`Скан от: ${lastScanTime}`, t) : t('Анализ еще не запускался')}
              </span>
            </div>
                      ...
                      <div className="space-y-1 bg-transparent border-0 p-0 m-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`px-2 py-0.5 rounded text-[8.5px] font-bold uppercase tracking-wider ${
                            ...
                            {isPatched ? t('Исправлено ✓') : isCritical ? t('Критическая угр.') : isWarning ? t('Риск ИБ') : t('Инфо')}
                          </span>
                          <span className="text-[10.5px] text-slate-400 font-mono">{t('Объект')}: {v.assetName}</span>
                        </div>
                        <h4 className={`font-bold text-sm ${isPatched ? 'text-slate-400 line-through' : 'text-slate-200'}`}>{translateSecurityText(v.title, t)}</h4>
                        <p className="text-[11px] text-slate-400 leading-relaxed">{translateSecurityText(v.description, t)}</p>
                      </div>
