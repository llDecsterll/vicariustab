...
import { User, Lock, Eye, EyeOff, ShieldCheck, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function LoginScreen({ onLogin, workspaceName, siteLogo, setupCompleteMessage }: LoginScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!auth) {
      setError('Неверный логин или пароль. Проверьте учётные данные.');
      return;
    }
      ...
      >
        <div className="bg-slate-800/95 backdrop-blur-md py-8 px-6 sm:px-10 rounded-3xl border border-slate-700/60 shadow-2xl relative">
          ...
          {setupCompleteMessage && (
            <div className="mb-4 p-3 bg-emerald-500/15 border border-emerald-500/30 rounded-xl flex items-start gap-2.5 text-emerald-400 text-[11px] leading-snug">
              <CheckCircle2 size={15} className="shrink-0 mt-0.5 text-emerald-500" />
              <span>{setupCompleteMessage}</span>
            </div>
          )}
