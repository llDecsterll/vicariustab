...
import { ComputerItem, ComputerCategory, ComputerStatus, EmployeeItem, ObjectItem, NetworkDevice, WarehouseItem } from '../types';
...
import {
  ...
  getInventoryPrefixForCategory,
  computerMatchesSearch,
  type InventoryRegistryContext,
...
export default function ComputersView({
  ...
  const [monitorSort, setMonitorSort] = useState<'none' | 'asc' | 'desc'>('none');
  ...
  const handleOpenAdd = () => {
    ...
    const registry: InventoryRegistryContext = {
      warehouseItems,
      computers: allComputers || computers,
      networkDevices,
    };
    const prefix = getInventoryPrefixForCategory(cat);
    setInventoryNumber(generateNextInventoryNumber(prefix, registry));
  ...
  const handleOpenEdit = (comp: ComputerItem) => {
    ...
    setMonitorDiagonalInput(
      comp.monitorDiagonalInches != null && comp.monitorDiagonalInches > 0
        ? String(comp.monitorDiagonalInches)
        : ''
    );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const monitorDiagonal = parseFloat(monitorDiagonalInput.replace(',', '.'));
    const payload = {
      category,
      deviceType,
      model: trimmedModel,
      ...
      monitorDiagonalInches:
        (deviceType === 'Монитор' || category === 'Монитор') &&
        Number.isFinite(monitorDiagonal) &&
        monitorDiagonal > 0
          ? monitorDiagonal
          : undefined,
      invoiceInfo,
  ...
  const filtered = computers.filter(c => {
    const matchesSearch = !search.trim() || computerMatchesSearch(c, search);
  ...
  const isMonitorRow = (c: ComputerItem) =>
    c.deviceType === 'Монитор' || c.category === 'Монитор';

  const displayList =
    equipmentTab === 'peripherals' && monitorSort !== 'none'
      ? [...filtered].sort((a, b) => {
          const monA = isMonitorRow(a);
          const monB = isMonitorRow(b);
          if (!monA && !monB) return 0;
          if (!monA) return 1;
          if (!monB) return -1;
          const da = a.monitorDiagonalInches ?? 0;
          const db = b.monitorDiagonalInches ?? 0;
          if (da === db) return a.model.localeCompare(b.model, 'ru');
          return monitorSort === 'asc' ? da - db : db - da;
        })
      : filtered;

  const showMonitorDiagonalColumn = equipmentTab === 'peripherals';
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        ...
        {equipmentTab === 'peripherals' && (
          <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-slate-50">
            <span className="text-[10px] font-bold text-slate-500 uppercase">{t('Мониторы:')}</span>
            <select
              value={monitorSort}
              onChange={(e) => setMonitorSort(e.target.value as 'none' | 'asc' | 'desc')}
              className="px-2.5 py-1.5 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 bg-white cursor-pointer"
            >
              <option value="none">{t('Сортировка: по умолчанию')}</option>
              <option value="asc">{t('Диагональ ↑')}</option>
              <option value="desc">{t('Диагональ ↓')}</option>
            </select>
          </div>
        )}
      </div>
          ...
          <table className="w-full min-w-[760px] text-left text-sm border-collapse">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50 text-slate-400">
                ...
                {showMonitorDiagonalColumn && (
                  <th className="py-3 px-5 font-semibold text-slate-500">{t("Диагональ")}</th>
                )}
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
                  ...
                  <tr key={c.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    {showMonitorDiagonalColumn && (
                      <td className="py-3.5 px-5 font-mono text-xs text-slate-600">
                        {isMonitorRow(c) && c.monitorDiagonalInches
                          ? `${c.monitorDiagonalInches}"`
                          : '—'}
                      </td>
                    )}
              ...
              {displayList.length === 0 && (
                <tr>
                  <td colSpan={showMonitorDiagonalColumn ? 9 : 8} className="text-center py-8 text-slate-400">{t("Оборудование не найдено по заданным фильтрам")}</td>
                </tr>
              )}
            </tbody>
          </table>
                ...
                {(deviceType === 'Монитор' || category === 'Монитор') && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full max-w-[160px] px-3 py-2 border border-slate-205 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-slate-700 font-mono"
                    />
                  </div>
                )}
