...
import { EQUIPMENT_TITLE_MAX_LENGTH, limitEquipmentTitle, networkDeviceMatchesSearch } from '../utils/equipmentFields';
import {
...
import { Network, Search, Edit2, ShieldAlert, Upload, FileText, RotateCcw, Trash2 } from 'lucide-react';
import ModalCloseButton from './ModalCloseButton';
import { CustomWarehouse, ComputerItem, NetworkDevice, NetworkDeviceType, ObjectItem, WarehouseItem } from '../types';
...
export default function NetworkView({
  ...
  const filtered = networkDevices.filter(dev => {
    const matchesSearch = !search.trim() || networkDeviceMatchesSearch(dev, search);
    const matchesCategory = filterCategory === 'Все' || dev.type === filterCategory;
    const displayStatus = getNetworkDeviceDisplayStatus(dev, warehouseItems, warehouses, objects);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {!isViewer && (
            <p className="text-[11px] text-slate-500 max-w-md md:max-w-xs md:text-right leading-snug">
