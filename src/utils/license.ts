...
export function validateKey(key: string): DecodedKeyDetails | null {
  ...
  const verified = verifySignedLicenseKeyClient(key, currentMac);
  if (!verified) return null;

  return {
    type: verified.type,
    expiresYear: verified.expiresYear,
    clientName: verified.clientName,
    clientEmail: verified.clientEmail,
    clientPhone: verified.clientPhone,
  };
}
...
/** Импорт/экспорт Excel склада — только при активированной лицензии (не в пробном периоде без ключа). */
export function isLicensedProduct(status: LicenseStatus): boolean {
  return status.isActivated;
}

export function canUseWarehouseExcel(status: LicenseStatus): boolean {
  return isLicensedProduct(status);
}

/** Резервное копирование (JSON / .enc) — только после активации ключа. */
export function canUseBackup(status: LicenseStatus): boolean {
  return isLicensedProduct(status);
}

/** Установка обновлений с GitHub — только после активации ключа. */
export function canUseSystemUpdate(status: LicenseStatus): boolean {
  return isLicensedProduct(status);
}
