...
**Автоматически** (интерактивно — домен + email):

```bash
npm run setup:domain
# или с параметрами:
node scripts/setup-domain.mjs --domain stack.example.com --email admin@example.com --ufw
```

С MySQL:

```bash
node scripts/setup-domain.mjs --domain stack.example.com --email admin@example.com --mysql --ufw
```

Скрипт: запишет `.env`, поднимет Docker + Caddy + Let's Encrypt.
