...
Вы можете выбрать наиболее подходящий вам способ запуска Uvwstack. **Способ А (Docker)** является наиболее предпочтительным, так как полностью изолирует приложение и исключает конфликты глобальных зависимостей Node.js.
...
# Сборка образа и фоновый запуск контейнера Uvwstack
...
PM2 будет следить за тем, чтобы узел Uvwstack работал 24/7 и автоматически поднимался при перезагрузках операционной системы:
```bash
...
База данных Uvwstack может работать в режиме локального шифрования файлов (по умолчанию), но для промышленного использования, стабильного многопользовательского доступа с разных компьютеров и отказоустойчивости рекомендуется использовать полноценную СУБД.
...
По умолчанию MySQL слушает внутреннюю петлю `127.0.0.1`. Изменим настройки на `0.0.0.0`, чтобы Uvwstack из Docker контейнера или другие филиалы могли подключиться:
```bash
...
## 🔗 Шаг 4. Подключение СУБД через интерфейс Uvwstack

После успешной установки веб-интерфейса и базы данных, вам остается связать их через настройки:

1. Откройте Uvwstack в вашем браузере: `http://<IP-АДРЕС_СЕРВЕРА>:8080` (или `8080` в зависимости от вашего порта).
...
**Причина:** Клонирование произведено рекурсивно или внутри уже существующей директории Uvwstack, из-за чего создалась вложенная структура `~/Uvwstack/Uvwstack`.
**Решение:** Проверьте ваше текущее местоположение:
```bash
pwd
# Если вы находитесь в ~/Uvwstack/Uvwstack — скопируйте файлы на один уровень выше или запустите Docker Compose из папки, где действительно хранится Dockerfile.
```
...
## 🚀 Готово!
Теперь ваша система учета IT-активов **Uvwstack** развернута в надежном, защищенном промышленном режиме и сохраняет все данные с автоматическим шифрованием! Любой пользователь с любого браузера или компьютера сможет получить санкционированный доступ к актуальной информации.
