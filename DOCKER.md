# 🐳 Документация по развертыванию Orbit на Ubuntu Server (Docker, PM2 & СУБД)

Данное руководство содержит исчерпывающие инструкции по установке полнофункциональной инфраструктуры системы **Orbit** с поддержкой реляционных СУБД (MySQL / PostgreSQL) и шифрованием данных.
...
Вы можете выбрать наиболее подходящий вам способ запуска Orbit. **Способ А (Docker)** является наиболее предпочтительным, так как полностью изолирует приложение и исключает конфликты глобальных зависимостей Node.js.
...
# Сборка образа и фоновый запуск контейнера Orbit
...
PM2 будет следить за тем, чтобы узел Orbit работал 24/7 и автоматически поднимался при перезагрузках операционной системы:
```bash
...
База данных Orbit может работать в режиме локального шифрования файлов (по умолчанию), но для промышленного использования, стабильного многопользовательского доступа с разных компьютеров и отказоустойчивости рекомендуется использовать полноценную СУБД.
...
По умолчанию MySQL слушает внутреннюю петлю `127.0.0.1`. Изменим настройки на `0.0.0.0`, чтобы Orbit из Docker контейнера или другие филиалы могли подключиться:
```bash
...
## 🔗 Шаг 4. Подключение СУБД через интерфейс Orbit

После успешной установки веб-интерфейса и базы данных, вам остается связать их через настройки:

1. Откройте Orbit в вашем браузере: `http://<IP-АДРЕС_СЕРВЕРА>:8080` (или `8080` в зависимости от вашего порта).
...
4. Заполните данные вашей запущенной СУБД:
   ...
   * **Хост / IP:** Укажите IP-адрес вашего сервера Linux. Если база данных установлена на том же сервере, где крутится контейнер Orbit Docker, укажите сетевой мост Docker: `172.17.0.1` или IP-адрес самого сервера во внутренней сети.
...
**Причина:** Клонирование произведено рекурсивно или внутри уже существующей директории Orbit, из-за чего создалась вложенная структура `~/Orbit/Orbit`.
**Решение:** Проверьте ваше текущее местоположение:
```bash
pwd
# Если вы находитесь в ~/Orbit/Orbit — скопируйте файлы на один уровень выше или запустите Docker Compose из папки, где действительно хранится Dockerfile.
```
...
## 🚀 Готово!
Теперь ваша система учета IT-активов **Orbit** развернута в надежном, защищенном промышленном режиме и сохраняет все данные с автоматическим шифрованием! Любой пользователь с любого браузера или компьютера сможет получить санкционированный доступ к актуальной информации.
