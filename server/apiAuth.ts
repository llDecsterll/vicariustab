...
/** Резервное копирование и импорт .enc — только при активированной лицензии (не пробный период без ключа). */
export async function requireActivatedLicense(
  _req: AuthedRequest,
  res: Response,
  next: NextFunction
): Promise<void> {
  const { data } = await loadApplicationData();
  const license = evaluateLicenseFromState(data);
  if (!license.isActivated) {
    res.status(402).json({
      error: "Резервное копирование доступно только после активации лицензии",
