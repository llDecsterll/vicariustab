...
<p align="center">
  <img src="docs/screenshots/en/03-computers.png" alt="Computer inventory" width="720">
  <br><em>Computers and equipment</em>
</p>
...
- [Installation](#-installation)
  ...
  - [Domain binding & HTTPS](#-domain-binding--https-automatic)
...
- the **trial period** starts (or the license already on that PC applies if it was activated there before);
- the **old activation key is not transferred** — request a **new key** for the new server MAC;
- **you do not need both keys** — only the key for the new machine.
...
# 🌐 Domain binding & HTTPS (automatic)

To access Vicariustab via a public URL (`https://stack.example.com`), use the built-in setup script — it writes `.env`, starts Docker, and configures Caddy with Let's Encrypt.

### Requirements

- VPS with a public IP (Ubuntu 20.04+)
...
You will be prompted for the **domain** and **owner email** (Let's Encrypt). The script checks DNS and starts containers.
...
| `--ufw` | Open ports 80/443 in firewall (Linux) |
| `--check-only` | Checks only, no Docker start |
...
Then open **`https://your-domain`** and complete [first launch](#-first-launch).

> **Manual setup:** see [`DOCKER.md`](./DOCKER.md) and `docker-compose.caddy.yml`.  
> **Keyserver** is not required on the server — license keys are issued separately by the rights holder (locally).
...
Edit `.env` and set a strong `DB_ENCRYPTION_KEY` before starting (required in production):

```bash
nano .env
```

**Recommended** — use the bundled PM2 config (`deploy/ecosystem.config.cjs`; loads `.env` via the server on startup):
...
> The application reads `.env` from the project root on startup. Docker Compose also uses `.env` for variable substitution.
...
```bash
cp .env.example .env   # optional; PORT defaults to 3000 in dev if unset
npm install
npm run dev
```
...
For the full audit test suite (server must be running):
...
| `STACK_DISABLE_AUTO_UPDATE` | Set `true` to disable GitHub auto-update on the server |
| `PM2_APP_NAME` | PM2 process name (default `vicariustab-system`) |
| `TELEGRAM_BOT_TOKEN` | Telegram bot for new-device login alerts |
| `RESEND_API_KEY` | Resend API key for email login alerts |
| `SESSION_ALERT_FROM_EMAIL` | Sender address for security emails |
| `STACK_DOMAIN` | Public domain (Docker + Caddy / reverse proxy) |
| `STACK_PUBLIC_URL` | Full public URL, e.g. `https://stack.example.com` |
| `STACK_TLS_EMAIL` | Let's Encrypt contact email |
| `TRUST_PROXY` | Set `true` behind Nginx/Caddy |
| `STACK_FORCE_HTTPS` | Redirect HTTP to HTTPS when `true` |
...
Or, if started manually: `pm2 restart vicariustab-system`
